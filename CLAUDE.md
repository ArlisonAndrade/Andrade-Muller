...
- `bank/web/` — Next.js 16 + Tailwind v4, mesmo padrão de `web/` mas com tokens próprios em `bank/web/app/globals.css` (serifa Source Serif 4 nos números de destaque, cards com borda 0.5px sem sombra, zero gradiente — não usar as cores/fontes do FM Gestão aqui). Sem template de UI externo — construído direto a partir do `design_tokens.md` (o template React/HTML cogitado no kickoff nunca foi escolhido). `.env.local` aponta pro mesmo projeto de `web/.env.local.example`.
...
- Deploy: Vercel para `bank/web` (projeto Vercel novo, mas Root Directory = `bank/web` no mesmo repo; env vars = mesmas do FM Gestão) e EAS Build para `bank/mobile` (`bank/mobile/eas.json`, bundle id `com.andrademuller.bank`) — ambos exigem login nas contas do usuário, não automatizável por aqui.
