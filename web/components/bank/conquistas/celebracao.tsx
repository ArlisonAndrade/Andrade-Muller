...
import type { Nivel } from "@/lib/bank/conquistas-visual";

export type ConquistaParaCelebrar = { codigo: string; emoji: string; nivel: Nivel; nome: string; frase: string };
...
export function CelebracaoConquistas({ conquistas }: { conquistas: ConquistaParaCelebrar[] }) {
  ...
  useEffect(() => {
    if (!aberta || !canvas.current) return;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ animation: "fade-slide-in 0.35s ease-out both" }}
      >
        <p className="text-xs font-semibold uppercase tracking-widest text-bank-primaria">
          {conquistas.length > 1 ? `Conquista ${indice + 1} de ${conquistas.length}` : "Conquista desbloqueada"}
        </p>
        <div className="relative mx-auto my-6 flex h-32 w-32 items-center justify-center" style={{ perspective: 600 }}>
          <span
            ...
            style={{ background: "radial-gradient(circle, rgba(250,204,21,0.6), transparent 70%)", animation: "brilho-pulsa 2s ease-in-out infinite" }}
          />
          <span key={atual.codigo} className="anima-medalha" style={{ animation: "medalha-entra 1.1s cubic-bezier(.2,.8,.2,1) both" }}>
            <Medalha emoji={atual.emoji} nivel={atual.nivel} tamanho={112} />
          </span>
        </div>
        <p className="text-xl font-semibold text-text-primary">{atual.nome}</p>
        <p className="mt-2 text-sm text-text-secondary">{atual.frase}</p>
        <button
          type="button"
        ...
        >
          {ultima ? "Guardar na coleção" : "Próxima conquista"}
        </button>
      </div>
    </div>
  );
